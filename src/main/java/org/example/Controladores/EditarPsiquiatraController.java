...
public class EditarPsiquiatraController<Psquiatra> implements Initializable {
    ...
    public void editarPsiquiatra(ActionEvent event) throws IOException {
        ...
        ListIterator<Psiquiatra> iteratorApellido = Psiquiatra.psiApell.get(nuevoPsiquiatra.apellidos).listIterator();
        while (iteratorApellido.hasNext()) {
            Psiquiatra pApellidoIterator = iteratorApellido.next();
            System.out.println(pApellidoIterator);
            if (pApellidoIterator.idPsiquiatra.equals(nuevoPsiquiatra.idPsiquiatra)) {
                iteratorApellido.set(nuevoPsiquiatra);
                break;
            }
        }
        ListIterator<Psiquiatra> iteratorEdad = Psiquiatra.psiEdad.get(nuevoPsiquiatra.edad).listIterator();
        while (iteratorEdad.hasNext()) {
            Psiquiatra pEdadIterator = iteratorEdad.next();
            System.out.println(pEdadIterator);
            if (pEdadIterator.idPsiquiatra.equals(nuevoPsiquiatra.idPsiquiatra)) {
                iteratorApellido.set(nuevoPsiquiatra);
                break;
            }
        }
